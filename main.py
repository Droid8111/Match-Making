...
# --- USER IDENTIFICATION DEPENDENCY ---
async def get_current_user_id(credentials: HTTPAuthorizationCredentials = Depends(security)) -> str:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Session security token has expired.")
...
async def verify_admin_access(credentials: HTTPAuthorizationCredentials = Depends(security)) -> bool:
    """
    Decodes the token using our JWT Secret and ensures the token claim role is strictly 'service_role'.
    """
    token = credentials.credentials
    try:
        ...
        raise HTTPException(
            ...
            detail="Access Denied: Insufficient permissions. Requires service_role authority."
        )
    except jwt.InvalidTokenError:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid or corrupted admin token key.")
...
async def get_current_match(user_id: str = Depends(get_current_user_id)):
    match_query = """
        ...
        WHERE r.status = 'active' AND m.status = 'paired' AND (:user_id = m.user_one_id OR :user_id = m.user_two_id)
        LIMIT 1;
    """
    ...
    profile_query = """
        ...
        GROUP BY p.user_id;
    """
    profile_record = await database.fetch_one(query=profile_query, values={"target_id": target_id})
...
async def update_preferences(prefs: PreferenceUpdate, user_id: str = Depends(get_current_user_id)):
    query = """
        ...
        VALUES (:user_id, :min_age, :max_age, :max_distance_km, NOW())
